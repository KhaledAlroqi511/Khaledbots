...
const youtube = new YouTube("AIzaSyBMkxgSIVtq0FymZ73IX_rO7ApKlYoqwP4"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyBMkxgSIVtq0FymZ73IX_rO7ApKlYoqwP4"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on('ready', () => {
    client.user.setActivity(`Lux Server`,{ type: 'WATCHING' });
})
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "new")) {
    const reason = message.content
      ...
      .slice(1)
      .join(" ");
    if (!message.guild.roles.exists(gg => gg.name === "Admins"))
    ...
    message.guild
      ...
      .then(c => {
        let role = message.guild.roles.find(gg => gg.name === "Admins");
        ...
        const embed = new Discord.RichEmbed()
          ...
          .addField(
            ...
            `يرجى محاولة شرح سبب فتح هذه التذكرة بأكبر قدر ممكن من التفاصيل. سيكون **فريق الدعم** لدينا هنا قريبًا للمساعدة.`
          )
          .setTimestamp();
        c.send({
          embed: embed
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@337213490626560000>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("guildMemberAdd", async member => {
  ...
  if (channel) {
    ...
    const wlcImage = await fsn.readFile("./Welcome10.png"); //اسم الصورة
    ...
    //Avatar
    var imageX = 120; //X coordinate
    var imageY = 130; //Y coordinate
    var imageRadius = 96; //نصف قطر الصورة الدائرية

    //Member Name
    var nameSize = "19pt"; //حجم خط الاسم
    var nameKind = "Source Sans Pro (OT1)"; //نوع خط الاسم
    var nameColor = "#070000"; //لون خط الاسم

    //Name Position
    var nameX = 350; //position x
    var nameY = 140; //position y
              ...
              m.channel
                ...
                .then(m2 => {
                  ...
                  message.channel
                    ...
                    .then(m => {
                      m.edit(message.member + ", **كم من الوقت تتفاعل🎙**");
                      setTimeout(() => {
                        m.delete();
                      }, 10000);
                      m.channel
...
client.on('message', function(message) {
    ...
    client.channels.get("703958769050320995").send({embed:iiMo});
    }
});
...
////شكر خاص لـ سرحان ولوفي ومرتجى على المساعدة لو لله وهم ماكان خلصنا هذا البوت الرهيب وما انسى